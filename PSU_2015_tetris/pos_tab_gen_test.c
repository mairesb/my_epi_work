...
** Last update Thu Mar 10 11:46:32 2016 baptiste mairesse
*/
  ...
  while (++i < height)
    {
      ...
      while (++j < height)
	{
	  if (i + all->pos_cur_tet.y < all->param.rows &&
	      j + all->pos_cur_tet.x < all->param.cols)
	    {
	      if (tab[i][j] != '0' && all->param.map[i + all->pos_cur_tet.y]
...
int		test_pos_turn_inv(t_all *all)
{
  ...
  tab[j] = NULL;
  return(test_pos(all, all->pos_cur_tet.width, all->pos_cur_tet.height, tab));
}

int		tab_clean_inv(t_all *all)
{
  char 		**tab;
  int 		i;
  int 		j;
  ...
  return(test_pos(all, all->pos_cur_tet.height, all->pos_cur_tet.width, tab));
}

int 		tab_clean(t_all *all)
{
  char 		**tab;
  int 		i;
  int 		j;
