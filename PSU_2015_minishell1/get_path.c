...
** Last update Tue Mar 15 09:42:58 2016 baptiste mairesse
*/
...
char		**get_path(char **env)
{
  ...
  var.k = 0;
  var.l = 0;
  while (!(path_line = src_env(env, "PATH")))
      free(path_line);
  path = my_malloc(sizeof(char *) * (nb_path(path_line) + 1));
  while (var.j == -1 || path_line[var.j] != '\0')
    {
